-- Replace the transitional wrapper with one unambiguous, atomic manual-order
-- function. Every field is stored before the paid job becomes dispatchable.

begin;

drop function public.create_admin_manual_order(
  uuid, integer, integer, text, text, text, text[], text, text, text,
  text, text, timestamptz, text, text, text, text[], text, text
);

drop function public.create_admin_manual_order_core(
  uuid, integer, integer, text, text, text, text[], text, text, text,
  text, text, timestamptz
);

create function public.create_admin_manual_order(
  p_offering_id uuid,
  p_quantity integer,
...
declare
  ...
  v_beneficiaries text[] := coalesce(p_beneficiary_names, '{}');
  v_reference text;
...
begin
  ...
  if btrim(coalesce(p_customer_email, '')) !~* '^[^[:space:]@]+@[^[:space:]@]+\.[^[:space:]@]+$'
    or char_length(btrim(p_customer_email)) > 254 then
  ...
  if btrim(coalesce(p_beneficiary_country, '')) = ''
    or char_length(btrim(p_beneficiary_country)) > 100
    ...
    or char_length(btrim(coalesce(p_dedication, ''))) > 300
    or char_length(btrim(coalesce(p_dedication_arabic, ''))) > 500
    or char_length(btrim(coalesce(p_dedication_remarks, ''))) > 2000
    or char_length(btrim(coalesce(p_payment_method, ''))) > 80
    or char_length(btrim(coalesce(p_notes, ''))) > 2000 then
    raise exception 'One or more job details are missing or too long';
  end if;
  ...
  if cardinality(v_beneficiaries) > 50 or exists (
    select 1 from unnest(v_beneficiaries) as beneficiary_name
  ...
  from unnest(v_beneficiaries) with ordinality as names(beneficiary_name, position);

  select offering.* into v_offering
  from public.offerings offering
  where offering.id = p_offering_id and offering.active;
  ...
  if v_offering.service_type = 'korban' then
    ...
    if cardinality(v_names) <> v_quantity or exists (
      select 1 from unnest(v_names) as participant_name
      where btrim(coalesce(participant_name, '')) = '' or char_length(btrim(participant_name)) > 120
    ) then raise exception 'Add one participant name for each Korban package'; end if;
    select coalesce(array_agg(btrim(participant_name) order by position), '{}')
    into v_names
    from unnest(v_names) with ordinality as names(participant_name, position);
  ...
  insert into public.orders (
    ...
    participant_names, dedication, dedication_arabic, dedication_remarks, notes,
    ...
    settlement_status, beneficiary_country, beneficiary_state, beneficiary_village,
    partner_organisation, beneficiary_names, completion_deadline, currency, entry_source
  ) values (
    v_reference, null, v_offering.id, v_offering.service_type, v_offering.category_slug, v_quantity,
    v_names, nullif(btrim(coalesce(p_dedication, '')), ''),
    nullif(btrim(coalesce(p_dedication_arabic, '')), ''),
    nullif(btrim(coalesce(p_dedication_remarks, '')), ''),
    ...
    btrim(p_beneficiary_country), nullif(btrim(coalesce(p_beneficiary_state, '')), ''),
    nullif(btrim(coalesce(p_beneficiary_village, '')), ''),
    nullif(btrim(coalesce(p_partner_organisation, '')), ''), v_beneficiaries,
    p_completion_deadline, 'SGD', 'admin_manual'
