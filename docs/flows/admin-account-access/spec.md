...
Let an authorized administrator create a staff account that works immediately, without depending on an emailed invitation link.
...
- Deliberately omitted: invitation tokens, temporary passwords sent by email, custom password-reset infrastructure, and a second onboarding wizard.
...
| Ready | Share credentials through a secure channel | Administrator signs in |
...
- [x] Creating an account does not send or require an invitation link.
