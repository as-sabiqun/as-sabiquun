...
export default async function VendorJobDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!user) redirect(`/partner-login?next=/vendor-dashboard/jobs/${id}`);

  const [{ data: assignedOrder, error: assignedError }, { data: offeredOrder, error: offeredError }] = await Promise.all([
    supabase
