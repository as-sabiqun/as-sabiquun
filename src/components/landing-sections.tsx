...
export function SectionEyebrow({ arabic, english }: { arabic: string; english: string }) {
  return (
    <div className="mb-6 inline-flex items-center gap-3">
      <span className="arabic text-[18px] font-medium leading-none" style={{ color: "var(--gold)" }}>{arabic}</span>
      <span className="h-3 w-px" style={{ backgroundColor: "rgba(162,124,71,0.4)" }} />
      <span className="text-[10px] font-bold uppercase tracking-[0.32em]" style={{ color: "var(--gold)", opacity: 0.85 }}>{english}</span>
    </div>
  );
}
  ...
  return (
    <section className="lp-hero-canvas relative flex min-h-[calc(100vh-88px)] flex-col overflow-hidden">
      <div className="pointer-events-none absolute inset-0 z-[2] overflow-hidden" aria-hidden="true">
        <span className="lp-orb" style={{ left: "12%", top: "20%", width: 340, height: 340, background: "radial-gradient(circle, rgba(162,124,71,0.30) 0%, transparent 70%)" }} />
        ...
        <span className="lp-orb lp-orb-c" style={{ left: "40%", bottom: "6%", width: 380, height: 380, background: "radial-gradient(circle, rgba(162,124,71,0.22) 0%, transparent 70%)" }} />
      </div>
      ...
      <div className="relative z-10 flex flex-1 flex-col items-center justify-center px-6 text-center">
        <p className="arabic mb-4 text-2xl" style={{ color: "var(--gold)" }} lang="ar" dir="rtl">بِالْأَمَانَةِ وَالْإِحْسَانِ</p>
        <h1 className="display max-w-2xl font-medium" style={{ fontSize: "clamp(40px, 6.5vw, 76px)", lineHeight: 1, color: "var(--teal-dark)", textShadow: "0 2px 18px rgba(255,253,250,0.45)" }}>
          Islamic services,{" "}
          <em className="display font-medium" style={{ color: "var(--gold)" }}>clearly carried through.</em>
        </h1>
        <p className="mx-auto mt-5 max-w-lg text-[16px] font-medium leading-relaxed lg:text-[17px]" style={{ color: "var(--ink)", textShadow: "0 1px 12px rgba(255,253,250,0.5)" }}>
...
export function AmanahShowcase() {
  ...
  return (
    <section id="record" className="relative overflow-hidden px-6 py-24 lg:px-10 lg:py-32" style={{ backgroundColor: "var(--white)" }}>
      <KhatimTexture color="var(--gold)" opacity={0.05} />
      <div className="relative mx-auto grid max-w-6xl grid-cols-1 items-center gap-12 lg:grid-cols-2 lg:gap-16">
        <Reveal>
          <div className="mb-6 inline-flex items-center gap-3">
            <span className="arabic text-[18px] font-medium leading-none" style={{ color: "var(--gold)" }}>اُنْظُرْ</span>
            <span className="h-3 w-px" style={{ backgroundColor: "rgba(162,124,71,0.4)" }} />
            <span className="text-[10px] font-bold uppercase tracking-[0.32em]" style={{ color: "var(--gold)" }}>See a live record</span>
          </div>
          <h2 className="display font-medium" style={{ fontSize: "clamp(32px, 4.5vw, 50px)", lineHeight: 1.06 }}>
            You don&apos;t just place an order.{" "}
            <em className="display font-medium" style={{ color: "var(--gold)" }}>You follow it.</em>
          </h2>
        ...
        <Reveal delay={120} className="card card-dark p-6 md:p-7">
          <div className="flex items-center justify-between border-b pb-5" style={{ borderColor: "rgba(255,253,250,.14)" }}>
            <div>
              ...
              <p className="arabic mt-1 text-lg" style={{ color: "var(--gold)" }} lang="ar" dir="rtl">وقف القرآن</p>
            </div>
      ...
      <div className="mx-auto grid max-w-6xl grid-cols-2 gap-6 lg:grid-cols-4 lg:gap-10">
        {stats.map((s, i) => (
          <Reveal key={s.label} delay={i * 90} className="text-center lg:text-left">
            <div className="display mb-2 text-[44px] font-medium leading-none lg:text-[56px]" style={{ color: "var(--gold)" }}>{s.num}</div>
            <div className="display text-[14px] font-medium leading-snug" style={{ color: "var(--ink)" }}>{s.label}</div>
...
export function WhyDifferent() {
  const pillars = [
    { arabicTitle: "الوُضُوح", title: "Clear scope, first", body: "See the service, the demonstration price, and what's included before you share a single detail. No surprises between request and receipt.", icon: <Sparkles size={22} style={{ color: "var(--gold)" }} />, tags: ["Scope upfront", "No hidden steps"] },
    { arabicTitle: "الأَمَانَة", title: "Human coordination", body: "A team reviews every request and manages the handoff to a fulfilment partner - your order is never just a queue position.", icon: <HandHeart size={22} style={{ color: "var(--gold)" }} />, tags: ["Reviewed by people", "Assigned, not automated"] },
    { arabicTitle: "الإِحْسَان", title: "Documented proof", body: "Available photos or video are reviewed before they're returned to you, so completion means something you can actually see.", icon: <ShieldCheck size={22} style={{ color: "var(--gold)" }} />, tags: ["Reviewed evidence", "Kept with your order"] },
  ];
  return (
    <section id="why" className="relative px-6 py-24 lg:px-10 lg:py-36">
      <div className="mx-auto max-w-6xl">
        ...
        <h2 className="display max-w-3xl font-medium" style={{ fontSize: "clamp(32px, 4.5vw, 48px)", lineHeight: 1.08 }}>
          Most platforms bolt Islamic services onto a generic form.{" "}
          <em className="display font-medium" style={{ color: "var(--gold)" }}>As-Sabiqun is built around the handoff.</em>
        </h2>
        ...
        <div className="mt-16 grid grid-cols-1 gap-5 md:grid-cols-3">
          {pillars.map((p, i) => (
            <Reveal key={p.title} delay={i * 110} className="card flex min-h-[340px] flex-col p-6">
              ...
              <span className="arabic mb-3 text-[26px] font-medium leading-none" style={{ color: "rgba(162,124,71,0.55)" }}>{p.arabicTitle}</span>
              <h3 className="display mb-2 text-[22px] font-medium">{p.title}</h3>
  ...
  return (
    <section id="how" className="relative border-t px-6 py-24 lg:px-10 lg:py-36" style={{ borderColor: "var(--line)", backgroundColor: "var(--cream)" }}>
      <div className="mx-auto max-w-6xl">
        ...
        <h2 className="display max-w-3xl font-medium" style={{ fontSize: "clamp(32px, 4.5vw, 48px)", lineHeight: 1.08 }}>
          Three steps.{" "}
          <em className="display font-medium" style={{ color: "var(--gold)" }}>Then it&apos;s underway.</em>
        </h2>
        <p className="mt-6 max-w-2xl text-[16px] leading-relaxed" style={{ color: "var(--muted)" }}>
          ...
          {steps.map((s, i) => (
            <Reveal key={s.n} delay={i * 120} className="card card-dark relative p-7">
              <div className="mb-5 flex items-baseline justify-between">
                <span className="arabic text-[64px] font-medium leading-none" style={{ color: "rgba(162,124,71,0.85)" }}>{s.n}</span>
                {i < steps.length - 1 && <ArrowRight size={18} className="hidden translate-x-[14px] md:block" style={{ color: "rgba(162,124,71,0.4)" }} />}
              </div>
              <h3 className="display mb-2 text-[22px] font-medium">{s.title}</h3>
    ...
    <section className="relative overflow-hidden px-6 py-24 lg:px-10 lg:py-36" style={{ backgroundColor: "var(--cream-dark)" }}>
      ...
      <div className="relative mx-auto max-w-5xl">
        ...
        <h2 className="display max-w-3xl font-medium" style={{ fontSize: "clamp(32px, 4.5vw, 48px)", lineHeight: 1.08 }}>
          ...
          <em className="display font-medium" style={{ color: "var(--gold)" }}>handled with amanah.</em>
        </h2>
        <div className="mt-14 grid grid-cols-1 gap-5 md:grid-cols-3">
          {groups.map((c, i) => (
            <Reveal key={c.title} delay={i * 110} className="card flex min-h-[230px] flex-col p-6">
              <span className="arabic mb-4 text-[30px] font-medium leading-none" style={{ color: "rgba(162,124,71,0.6)" }} lang="ar" dir="rtl">{c.ar}</span>
              <div className="flex-1" />
              <h3 className="display mb-2 text-[20px] font-medium">{c.title}</h3>
        ...
        <Reveal delay={120} className="card card-dark mt-5 flex flex-col items-start gap-6 p-7 lg:flex-row lg:items-center lg:p-9">
          ...
          <div>
            <p className="display text-[18px] leading-snug lg:text-[20px]">
              ...
              <em style={{ color: "var(--gold)" }}>see</em> - clear scope, human coordination, documented proof.
            </p>
  ...
  return (
    <section id="services" className="relative border-t px-6 py-24 lg:px-10 lg:py-36" style={{ borderColor: "var(--line)" }}>
      <div className="mx-auto max-w-6xl">
        ...
        <h2 className="display max-w-3xl font-medium" style={{ fontSize: "clamp(32px, 4.5vw, 48px)", lineHeight: 1.08 }}>
          Four focused ways{" "}
          <em className="display font-medium" style={{ color: "var(--gold)" }}>to arrange something good.</em>
        </h2>
        <p className="mt-6 max-w-2xl text-[16px] leading-relaxed" style={{ color: "var(--muted)" }}>
          ...
          {services.map((service, i) => (
            <Reveal key={service.slug} delay={i * 80}>
              <Link href={service.href} className="card group flex items-center gap-5 p-6 transition-transform hover:scale-[1.01] active:scale-[0.99]">
                <div className="grid h-16 w-16 shrink-0 place-items-center rounded-2xl" style={{ backgroundColor: "rgba(162,124,71,0.1)", border: "1px solid var(--line)" }}>
                  <span className="arabic text-[22px] font-medium leading-none" style={{ color: "var(--gold)" }} lang="ar" dir="rtl">{service.arabic}</span>
                </div>
                <div className="min-w-0 flex-1">
                  <h3 className="display text-[18px] font-medium leading-snug">{service.title}</h3>
  ...
  return (
    <section id="faq" className="relative border-t px-6 py-24 lg:px-10 lg:py-36" style={{ borderColor: "var(--line)", backgroundColor: "var(--cream)" }}>
      <div className="mx-auto max-w-3xl">
        ...
        <h2 className="display mb-12 font-medium" style={{ fontSize: "clamp(32px, 4.5vw, 48px)", lineHeight: 1.08 }}>
          Things people <em className="display font-medium" style={{ color: "var(--gold)" }}>actually</em> ask.
        </h2>
          ...
          {faqs.map((f, i) => (
            <Reveal key={f.q} delay={i * 70} as="details" className="card group cursor-pointer p-5">
              <summary className="flex list-none items-center justify-between gap-4">
                <h3 className="display text-[16px] font-medium leading-snug lg:text-[17px]">{f.q}</h3>
  ...
  return (
    <section className="relative overflow-hidden border-t px-6 py-28 lg:px-10 lg:py-40" style={{ borderColor: "var(--line)" }}>
      ...
      <div className="relative mx-auto max-w-3xl text-center">
        <span className="arabic mb-6 block font-medium leading-none" style={{ fontSize: "clamp(60px, 9vw, 120px)", color: "var(--gold)", textShadow: "0 4px 30px rgba(0,0,0,0.15)" }} lang="ar" dir="rtl">اِبْدَأ</span>
        <p className="mb-8 text-[10px] font-bold uppercase tracking-[0.4em]" style={{ color: "rgba(162,124,71,0.85)" }}>ibda&apos; · begin</p>
        <h2 className="display font-medium" style={{ fontSize: "clamp(34px, 5vw, 52px)", lineHeight: 1.05 }}>
          Your service, clearly delivered.{" "}
          <em className="display font-medium" style={{ color: "var(--gold)" }}>What are you arranging today?</em>
        </h2>
